...
| `gh student accept <org>/<classroom>/<assignment>` | Accept an assignment: create a private repo from the template, add the student as `maintain`, write `.classroom50.yml`, and print clone instructions. Idempotent — re-runs after a partial failure resume rather than aborting on a 422 already-exists. |
| `gh student invite <org>/<repo> <user>`            | Invite a classmate or TA to the repo with `push` permission.      |
| `gh student submit`                                | Snapshot the current branch (plus the latest instructor `.gitignore` and `.github/`, when present) and force-push it to the assignment repo's default branch. Reads `.classroom50.yml` for the source repo and branch. |

Run `gh student <command> --help` for available flags. Errors always go to stderr with a non-zero exit code.
