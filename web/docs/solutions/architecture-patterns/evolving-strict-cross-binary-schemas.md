...
- `docs/solutions/architecture-patterns/copying-records-across-strict-cross-binary-contract.md` — the **record-COPY** variant on this same `assignments.json` contract. Where this doc covers _adding a field_, that one covers _cloning a whole record_ (spread the source to preserve unknown fields, deep-clone the modeled ones, `delete` undefined keys to stay omitempty-clean) under the identical strict round-trip hazard.
- `docs/solutions/developer-experience/capability-url-drift-protected-classroom-publishing.md` — the **path** contract sibling; together the five (data, schema-evolution, record-copy, name, path) form the cross-binary-contract cluster.
